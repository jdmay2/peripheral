...
| Tool        | Version | Notes                                                      |
| ----------- | ------- | ---------------------------------------------------------- |
| pnpm        | 10.28.2 | `node-linker=hoisted` in `.npmrc` — **required** for Metro |
| Turborepo   | 2.8.3   | `turbo build` with `^build` dep ordering + remote caching  |
| Changesets  | 2.29.7  | `"linked": [["@peripheral/*"]]`, `"access": "public"`      |
| builder-bob | 0.40.17 | ESM (`{"esm": true}`) + CJS + `.d.ts` to `lib/`            |
| TypeScript  | 5.7+    | Strict, `noUncheckedIndexedAccess`, bundler resolution     |
...
| `parsers/`  | heart-rate, blood-pressure, cycling (CSC + power), fitness-machine (FTMS), glucose, health-thermometer, environmental, pulse-oximeter, running (RSC), weight-scale, `registry.ts` |
| `profiles/` | 10 device profiles: HeartRateMonitor, CyclingPower, IndoorBike, Treadmill, Rower, RSC, WeightScale, BloodPressure, PulseOximeter, CSC                                             |
| `hooks/`    | `useBleManager()`, `useScan(filter?)`, `useDevice(id)`, `useCharacteristic(device, opts)`                                                                                         |
| `types/`    | ConnectionState enum, GATT UUID constants (StandardServices, StandardCharacteristics), 15+ parsed data interfaces                                                                 |
| `utils/`    | `bytes.ts` (DataView helpers, base64 decode), `ieee11073.ts` (SFLOAT/FLOAT decoders)                                                                                              |
...
| `devices/` | 10 typed abstractions: Light, Climate, Lock, Cover, Fan, MediaPlayer, Vacuum, AlarmPanel, Sensor, BinarySensor                                                    |
| `hooks/`   | `useHomeAssistant(config)`, `useEntity(id)`, `useEntities(filter?)`, `useAreas()`, `useService()`, `useAutomation()`, `useHistory(id, period)`, `useMQTT(config)` |
| `types/`   | `entities.ts`, `homeassistant.ts` (HA WS/REST), `mqtt.ts` (MQTT + Z2M + HA discovery), `homekit.ts` (HK hierarchy)                                                |
| `utils/`   | `service-calls.ts` (40+ builders: `lightTurnOn`, `climateSetTemperature`, `lockLock`, `vacuumStart`, etc.), `entities.ts` (filter/group/parse helpers)            |
...
- **Entity helpers** — `extractDomain()`, `filterByDomain()`, `groupByArea()`, `isStateActive()`, `numericState()`
...
Segmenter (sliding window 1–3s, 50% overlap, SMA onset + activity classifier)
    ▼
┌─ ThresholdClassifier (tap/shake/flick — always runs in parallel)
...
| Directory      | Contents                                                                                                                                                                           |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `classifiers/` | `threshold.ts` (tap/doubleTap/shake/flick via peak detection), `dtw.ts` (Sakoe-Chiba DTW, rotation-invariant, multi-template voting), `ml-inference.ts` (lazy TFLite/ONNX wrapper) |
...
| `engine/`      | `GestureEngine` — full orchestrator: preprocess → buffer → segment → classify → FP guard → emit                                                                                    |
...
| `hooks/`       | `useGestureEngine`, `useGestureRecognition`, `useGestureRecorder`, `useActivityContext`, `useGestureLibrary`, `useSensorVisualization`                                             |
...
| ---- | ------------------- | ------------------------------ | ----------- | ---------------------------- |
| 1    | ThresholdClassifier | None — configure thresholds    | ~90% simple | tap, doubleTap, shake, flick |
...
`GestureRecorder`: countdown (3s) → capture (2.5s × 5 reps) → auto-trim silence → DTW consistency check (≥70% to accept). Emits: `phaseChanged`, `countdownTick`, `recordingCompleted`, `sessionCompleted`.
...
- **`FalsePositiveGuard`** — `evaluate(result, activity)`, `arm()`, `disarm()`, `getMetrics()`
...
| Requirement   | Value                                                    |
| ------------- | -------------------------------------------------------- |
| React Native  | ≥ 0.73 (0.82+ mandates New Architecture)                 |
| React         | ≥ 18.0                                                   |
